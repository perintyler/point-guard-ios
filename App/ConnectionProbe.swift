...
enum ProbeOutcome: Equatable {
    ...
    /// Whether the app can actually read the book with this configuration.
    ...
    var message: String {
        ...
        case .working(let count):
            return "Connected and authorized — the book has \(count) session\(count == 1 ? "" : "s")."
        case .reachableButUnauthorized:
...
/// only route point-guard answers without a secret, so it separates "the server
/// is there" from "the secret is wrong"; `/book` then proves the credential
/// actually works and says how much came back. A probe that only called
/// `/health` would say "connected" while every real call 403s, and one that
/// only called `/book` would report an identical failure for a down server and
/// a bad secret.
struct ConnectionProbe {
    ...
    func run() async -> ProbeOutcome {
        ...
        do {
            let book = try await PointGuardClient(config: config, urlSession: urlSession).book()
            return .working(sessionCount: book.sessions.count)
        } catch let error as PointGuardError {
    ...
    static func classify(apiError error: PointGuardError) -> ProbeOutcome {
        switch error {
        case .badURL:
            return .badURL
        ...
        case .http(let status, let detail):
            // 403, not 401: point-guard's auth middleware returns forbidden for
            // a caller with no (or a wrong) bearer token.
